...
def test_acquired_data_independent_default_containers():
    a = AcquiredData(data=np.zeros((1, 1), dtype=np.float32), kind=DataKind.FLIM_RAW_FRAME)
    b = AcquiredData(data=np.zeros((1, 1), dtype=np.float32), kind=DataKind.FLIM_RAW_FRAME)

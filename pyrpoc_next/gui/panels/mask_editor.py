"""Interactive mask editor: draw freehand ROIs over a display's channels, threshold
them, and produce a binary mask for the mask modifier.

Ported from the original pyrpoc MaskEditorWidget (the editor the app actually used).
Self-contained Qt/cv2 — the output is a 2D uint8 {0,255} array, exactly what
MaskModifier.mask expects. The host dialog adds a display-source picker that pulls a
display's normalized [C,H,W] data (get_normalized_data_3d) as the backdrop to draw on.
"""
...
class MaskImageView(QGraphicsView):
    """Graphics view where a freehand polygon ROI is traced with the left mouse."""
...
class MaskEditorWidget(QWidget):
    """Draw freehand ROIs over channel data, threshold, and emit a binary mask."""
    ...
    def reset_threshold_controls(self) -> None:
        ...
        if high_default < low_default:
            high_default = low_default
        for widget in (self.low_spin, self.high_spin, self.low_slider, self.high_slider):
    ...
    def generate_mask(self) -> np.ndarray | None:
        ...
        for roi in self._rois:
            if len(roi.points) < 3:
                continue
            polygon = np.array([[int(round(x)), int(round(y))] for x, y in roi.points],
    ...
    def save_mask(self) -> None:
        ...
        if mask is None:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Mask", "",
                                              "PNG (*.png);;TIFF (*.tif *.tiff);;All Files (*)")
        if not path:
            return
        if not cv2.imwrite(str(Path(path)), mask):
...
class MaskEditorDialog(QDialog):
    """Hosts the mask editor with a picker to load a display's data as the backdrop."""

    def __init__(self, displays: list, parent: QWidget | None = None):
        super().__init__(parent)
        self.setWindowTitle("Mask Editor")
        self.mask: np.ndarray | None = None
        self.sources = [d for d in displays if hasattr(d, "get_normalized_data_3d")]

        root = QVBoxLayout(self)
        source_row = QHBoxLayout()
        source_row.addWidget(QLabel("Source display:"))
        self.source_combo = QComboBox()
        for display in self.sources:
            self.source_combo.addItem(display.manifest.display_name, display)
        load_btn = QPushButton("Load from display")
        load_btn.clicked.connect(self.load_from_display)
        source_row.addWidget(self.source_combo, 1)
        source_row.addWidget(load_btn)
        root.addLayout(source_row)

        self.editor = MaskEditorWidget()
        self.editor.create_mask_requested.connect(self.on_create)
        self.editor.cancel_requested.connect(self.reject)
        root.addWidget(self.editor, 1)
        self.resize(900, 600)

        if self.sources:
            self.load_from_display()

    def load_from_display(self) -> None:
        display = self.source_combo.currentData()
        if display is None:
            return
        data = display.get_normalized_data_3d()
        if data is not None and np.asarray(data).ndim == 3:
            self.editor.set_image_data(np.clip(np.asarray(data, dtype=np.float32), 0.0, 1.0) * 255.0)

    def on_create(self, mask) -> None:

...
def apply_morph(mask: np.ndarray, open_ksize: int = 3, close_ksize: int = 9) -> np.ndarray:
...
def components_to_labels(mask: np.ndarray, min_area: int = 120) -> np.ndarray:
...
def threshold_components(
    ...
    if hi < lo:
        lo, hi = hi, lo
    mask = ((img_u8 >= lo) & (img_u8 <= hi)).astype(np.uint8) * 255
    mask = apply_morph(mask, open_ksize=open_ksize, close_ksize=close_ksize)
    return components_to_labels(mask, min_area=min_area)
...
def watershed(
    ...
    sure_fg = (base > 0).astype(np.uint8) * 255
    ...
    if n_markers <= 1:
        return components_to_labels(sure_fg, min_area=min_area)
...
def segment(image: np.ndarray, method: str, **kwargs) -> np.ndarray:
    ...
    if name == "threshold + components":
        return threshold_components(image, **kwargs)
    if name == "watershed":
        return watershed(image, **kwargs)
...
def labels_to_contours(labels: np.ndarray, min_area: int = 120) -> list[np.ndarray]:
    ...
    for label_id in np.unique(arr):
        if int(label_id) <= 0:
            continue
        mask = (arr == label_id).astype(np.uint8) * 255

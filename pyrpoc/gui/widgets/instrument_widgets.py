...
class TimeTaggerInstrumentWidget(BaseInstrumentWidget):
    def __init__(self, instrument: "TimeTaggerInstrument", on_change=None, parent: QWidget | None = None) -> None:
...
def build_time_tagger_widget(instrument, parent, on_change) -> BaseInstrumentWidget:
    return TimeTaggerInstrumentWidget(instrument, on_change=on_change, parent=parent)


instrument_widget_builders: dict[str, Callable[..., BaseInstrumentWidget]] = {
    "time_tagger": build_time_tagger_widget,
}


def build_instrument_widget(instrument: BaseInstrument, parent: QWidget | None = None, on_change=None) -> BaseInstrumentWidget:
    builder = instrument_widget_builders.get(instrument.instrument_key)
    if builder is None:
        return BaseInstrumentWidget(instrument, on_change=on_change, parent=parent)
    return builder(instrument, parent, on_change)

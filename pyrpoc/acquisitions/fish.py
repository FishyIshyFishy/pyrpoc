...
from .local_rpoc import LocalRPOC
from PIL import Image


class Fish(Acquisition):
    def __init__(self, galvo=None, data_inputs=None, prior_stage=None, num_frames=1, signal_bus=None, acquisition_parameters=None, **kwargs):
        ...
        if self.galvo is None:
            raise ValueError("Galvo is required for Fish acquisition")
        if not self.data_inputs:
            raise ValueError("At least one DataInput is required for Fish acquisition")
        if self.prior_stage is None:
            raise ValueError("Prior stage is required for Fish acquisition")
    ...
    def configure_rpoc(self, rpoc_enabled, rpoc_mask_channels=None, rpoc_static_channels=None, rpoc_script_channels=None, **kwargs):
        ...
        if self.signal_bus:
            ...
            self.signal_bus.console_message.emit(f"Fish RPOC Configured - enabled: {rpoc_enabled}, masks: {n_masks}, static: {n_static}, script: {n_script}, total: {n_total}")
    ...
    def perform_acquisition(self):     
        ...
        for frame_idx in range(self.num_frames):
            ...
            for pos_idx, (x_step, y_step, z_step) in enumerate(stage_step_sizes):
                ...
                # Perform local RPOC treatment before imaging
                if self.rpoc_mask_channels:
                    self._perform_local_rpoc_treatment()
    ...
    def _perform_local_rpoc_treatment(self):
        """Perform local RPOC treatment using the first enabled mask channel"""
        if not self.rpoc_mask_channels:
            return
        
        # Get the first enabled mask channel
        enabled_mask_channels = {k: v for k, v in self.rpoc_mask_channels.items() if v.get('enabled', True)}
        if not enabled_mask_channels:
            return
        
        # Use the first enabled mask channel
        channel_id = list(enabled_mask_channels.keys())[0]
        channel_data = enabled_mask_channels[channel_id]
        mask_data = channel_data.get('mask_data')
        
        if mask_data is None:
            return
        
        # Create treatment parameters from acquisition parameters
        treatment_parameters = {
            'dwell_time': self.acquisition_parameters.get('local_rpoc_dwell_time', 10),  # Use local RPOC dwell time
            'extrasteps_left': self.acquisition_parameters.get('local_extrasteps_left', 50),
            'extrasteps_right': self.acquisition_parameters.get('local_extrasteps_right', 50),
            'amplitude_x': self.acquisition_parameters.get('amplitude_x', 0.5),
            'amplitude_y': self.acquisition_parameters.get('amplitude_y', 0.5),
            'offset_x': self.acquisition_parameters.get('offset_x', 0.0),
            'offset_y': self.acquisition_parameters.get('offset_y', 0.0),
            'x_pixels': self.acquisition_parameters.get('x_pixels', 512),
            'y_pixels': self.acquisition_parameters.get('y_pixels', 512),
            'offset_drift_x': self.acquisition_parameters.get('offset_drift_x', 0.0),
            'offset_drift_y': self.acquisition_parameters.get('offset_drift_y', 0.0),
            'repetitions': self.acquisition_parameters.get('repetitions', 1),
            'ttl_device': channel_data.get('device', 'Dev1'),
            'ttl_port_line': channel_data.get('port_line', 'port0/line0'),
            'pfi_line': self.acquisition_parameters.get('pfi_line', 'None')
        }
        
        # Create and perform local RPOC treatment
        local_rpoc = LocalRPOC(
            galvo=self.galvo,
            mask_data=mask_data,
            treatment_parameters=treatment_parameters,
            signal_bus=self.signal_bus,
            save_enabled=False  # Don't save treatment data
        )
        
        if self.signal_bus:
            self.signal_bus.console_message.emit(f"Starting local RPOC treatment at position...")
        
        # Perform the treatment
        local_rpoc.perform_acquisition()
        
        if self.signal_bus:
            self.signal_bus.console_message.emit(f"Local RPOC treatment completed")
    
    def collect_confocal_data(self, galvo, ai_channels):
        """Collect confocal data using the same logic as ConfocalMosaic"""
        try:
    ...
    def save_data(self, data):
        ...
        try:
            ...
            for input_ch_idx in range(num_channels):
                ...
                if self.signal_bus:
                    self.signal_bus.console_message.emit(f"Saved fish data for {input_channel_name}")
            
        except Exception as e:
            if self.signal_bus:
                self.signal_bus.console_message.emit(f"Error saving fish data: {e}")
            else:
                print(f"Error saving fish data: {e}")
    ...
    def generate_simulated_confocal(self):
        ...
        if self.signal_bus:
            ...
            n_static = len(self.rpoc_static_channels) if self.rpoc_static_channels else 0
            n_script = len(self.rpoc_script_channels) if self.rpoc_script_channels else 0
            self.signal_bus.console_message.emit(f"Fish RPOC Debug - enabled: {self.rpoc_enabled}, masks: {n_masks}, static: {n_static}, script: {n_script}")
        ...
        if self.rpoc_enabled and (self.rpoc_mask_channels or self.rpoc_static_channels):
            ...
            if self.signal_bus:
                ...
                self.signal_bus.console_message.emit(f"Fish RPOC Active - {len(self.rpoc_mask_channels)} masks, {len(self.rpoc_static_channels)} static channels: {', '.join(total_info)}")

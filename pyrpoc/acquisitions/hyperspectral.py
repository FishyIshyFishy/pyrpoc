...
class Hyperspectral(Acquisition):
    ...
    def save_data(self, data):
        """
        Save hyperspectral data as a single TIFF file
        ...
        try:
            ...
            if self.signal_bus:
                self.signal_bus.console_message.emit(f"Saved hyperspectral data to {filepath}")
            
        except Exception as e:
            if self.signal_bus:
                self.signal_bus.console_message.emit(f"Error saving hyperspectral data: {e}")
            else:
                print(f"Error saving hyperspectral data: {e}")